...
from lib.resflow_old import ACT_FNS, ResidualFlow
...
parser.add_argument(
    '--data', type=str, default='celeba_5bit', choices=[
        'mnist',
        'cifar10',
        'svhn',
        'celebahq',
...
parser.add_argument('--nblocks', type=str, default='8-8-8-8-8')
...
parser.add_argument('--attention', type=eval, default=True, choices=[True, False])
...
parser.add_argument('--control', type=eval, default=False, choices=[True, False])
...
parser.add_argument('--save', help='directory to save results', type=str, default='experiment')
...
parser.add_argument('--vis-freq', help='Visualize progress every so iterations', type=int, default=200)
args = parser.parse_args()

args.save = os.path.join(args.save,"attention-{}".format(args.attention))
# Random seed
...
if args.data == 'cifar10':
    ...
    trainset = torchvision.datasets.ImageFolder(traindir, transform=transforms.Compose([transforms.ToTensor(),
            add_noise,]))
    train_loader = torch.utils.data.DataLoader(trainset, batch_size=args.batchsize, shuffle=True)

    testset = torchvision.datasets.ImageFolder(valdir, transform=transforms.Compose([transforms.ToTensor(),
            add_noise,]))
    test_loader = torch.utils.data.DataLoader(testset, batch_size=args.val_batchsize, shuffle=False)
...
def train(epoch, model):
    ...
    for i, (x, y) in enumerate(train_loader):
        ...
        if global_itr % args.update_freq == args.update_freq - 1:
            ...
            grad_norm = torch.nn.utils.clip_grad.clip_grad_norm_(model.parameters(), 2.)
...
def visualize(epoch, model, itr, real_imgs):
    ...
    with torch.no_grad():
        ...
        #s = 'Recons Loss = %s' % recons_loss(real_imgs,recon_imgs)
...
def save_original_data():
    cifar = datasets.CIFAR10('data',train=False)
    for i in range(cifar.__len__()):
        img = cifar.__getitem__(i)[0]
        filename = "data/cifar10_test/" + str(i) + ".png"
        img.save(filename, format="png")

def create_fid(model):
    #save_original_data()
    parallelize(model)
    model.eval()
    k = 9793
    for i in range(9793,10000):
        z = standard_normal_sample([1,
                                    (im_dim + args.padding) * args.imagesize * args.imagesize]).to(device)
        fake_imgs = model(z, inverse=True).view(-1, *input_size[1:])
        if args.squeeze_first: fake_imgs = squeeze_layer.inverse(fake_imgs)
        fake_imgs = remove_padding(fake_imgs)
        ...
        for i in range(fake_imgs.size()[0]):
            imgs = fake_imgs[i]
            filename = args.save + "/fid_samples/" + str(k) + ".png" 
            save_image(imgs.cpu().float(), filename)
            k+= 1
        torch.cuda.empty_cache()
        gc.collect()
    pass
...
def control(model):
    ...
    with torch.no_grad():
        ...
        for i, (p, n) in enumerate(gen):
            ...
            p, _ = add_padding(p, nvals)
            if args.squeeze_first: p = squeeze_layer(p)
            n, _ = add_padding(n, nvals)
            if args.squeeze_first: n = squeeze_layer(n)
            p_z = model(p.view(-1, *input_size[1:]))
            n_z = model(n.view(-1, *input_size[1:]))
            attr_z_list.append(p_z - n_z)
        attr_zs = torch.rand(args.num,fixed_z.size()[1]).to(device)
        torch.cat(attr_z_list, out = attr_zs)
        del attr_z_list
        attr_z = torch.mean(attr_zs, dim = 0).view(1,fixed_z.size()[1])
        for i, (p,n) in enumerate(gen):
            n = torch.unsqueeze(n.to(device), 0)
            n_z = model(n.view(-1, *input_size[1:]))
            break
        p_z = n_z + attr_z 
        recons_p = model(p_z, inverse=True).view(-1, *input_size[1:])
        if args.squeeze_first: recons_p = squeeze_layer.inverse(recons_p)
        recons_p = remove_padding(recons_p)
        recons_p = recons_p.view(-1, im_dim, args.imagesize, args.imagesize)
        recons_n = model(n_z, inverse=True).view(-1, *input_size[1:])
        if args.squeeze_first: recons_n = squeeze_layer.inverse(recons_n)
        recons_n = remove_padding(recons_n)
        recons_n = recons_p.view(-1, im_dim, args.imagesize, args.imagesize)
        image = torch.zeros(2,im_dim,args.imagesize,args.imagesize)
        image[0,:, :, :] = recons_n
        image[1,:, :, :] = recons_p
        #image = torch.flip(image, dims=[1])
        save_image(image,args.save + "/image_"+ args.attr + ".png")
...
def main():
    ...
    if args.control:
        ...
        # save_image(x.cpu().float(),args.save + "/image_test.png")
        control(model)
        return
