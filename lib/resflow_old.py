...
class ResidualFlow(nn.Module):
    ...
    def __init__(
        ...
        self.transforms = self._build_net(input_size,attention)

        self.dims = [o[1:] for o in self.calc_output_size(input_size)]

        if self.classification:
            self.build_multiscale_classifier(input_size)
        #self.attention_map = L2_Self_Attn(intermediate_dim)

    def _build_net(self, input_size,attention):
        _, c, h, w = input_size
        transforms = []
    ...
    def _calc_n_scale(self, input_size):
        ...
        while h >= 4 and w >= 4:
            ...
            w = w // 2
        return n_scale
    def get_attention_map_ResidualFlow(self):
        return self.attention_map.get_attn_map()
...
class StackediResBlocks(layers.SequentialFlow):
    ...
    def __init__(
        ...
        # self.attention_map = L2_Self_Attn(idim)
        ...
        def _resblock(initial_size, fc, idim=idim, first_resblock=False,attention=True):
            if fc:
        ...
        if squeeze:
            ...
            for i in range(n_blocks):
                ...
                chain.append(_resblock(initial_size, fc, first_resblock=first_resblock and (i == 0),attention=attention))
            ...
            for _ in range(n_blocks):
                if quadratic: chain.append(_quadratic_layer(initial_size, fc))
                chain.append(_resblock(initial_size, fc,attention=attention))
                if actnorm: chain.append(_actnorm(initial_size, fc))
                if fc_actnorm: chain.append(_actnorm(initial_size, True))
            # Use four fully connected layers at the end.
            if fc_end:
                for _ in range(fc_nblocks):
                    chain.append(_resblock(initial_size, True, fc_idim,attention=attention))
...
class L2_Self_Attn(nn.Module):
    def __init__(self,in_dim):
        ...
        self.query_conv = nn.Conv2d(in_channels = in_dim , out_channels = in_dim//8 , kernel_size= 1) # W_Q
        self.value_conv = nn.Conv2d(in_channels = in_dim , out_channels = in_dim , kernel_size= 1) # W_V
    ...
    def compute_bound(self,N,D,W_Q,W_V):
        ...
        input_to_lam = (N/e).detach().cpu().numpy()
        #print(input_to_lam.dtype)
        phi = lambertw(input_to_lam).real
        phi = torch.from_numpy(phi).to(torch.float).to(W_Q.device)
        bound = torch.sqrt(torch.tensor(N/D,dtype=torch.float32))*(4*phi + 1)*torch.norm(W_Q)*torch.norm(W_V)
        return bound
    ...
    def forward(self,x):
        ...
        W_Q = W_Q1.view(1,self.query_conv.in_channels,self.query_conv.out_channels).repeat(m_batchsize,1,1)
        normed_query = self.row_wise_norm(proj_query)
        one_vec = self.one_vec.view(1,1).repeat(N,1).view(1,N,1).repeat(m_batchsize,1,1).to(x.device)#.cuda()
        ...
        scalar1 = -1/torch.sqrt(torch.tensor(C,dtype=torch.float32))

        P_before_softmax = (scalar1)*(P_first_term + P_third_term + P_second_term)
        ...
        A_half = self.query_conv(attention.view(m_batchsize,-1,width,height)).view(m_batchsize,N,-1)
        attention = (scalar2)*torch.bmm(A_half,W_Q.permute(0,2,1))

        out = self.value_conv(attention.view(m_batchsize,-1,width,height)).view(m_batchsize,N,-1)
        bound = self.compute_bound(width*height,C,W_Q1,W_V1)
        out /= bound
        map = out.view(m_batchsize,C,width,height)
        out = self.gamma*map + x
        del P_first_term
        del P_second_term
        del P_third_term
        del P_before_softmax
        del A_half, bound, map
